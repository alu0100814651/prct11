require 'nitz/codigo.rb'
require 'nitz/libreria_lista.rb'
require 'nitz/apa.rb'
require 'spec_helper'

describe APA do
    before :all do
         @biblio1 = Biblio.new(['Dave', 'Andy'], ['Thomas', 'Hunt'], 'Programming Ruby 1.9 & 2.0: The Pragmatic Programmers’ Guide', 'Pragmatic Bookshelf', 4, 'July 7 2013', ['9781937785499', '1937785491'], 'The Facets of Ruby')
         @biblio2 = Biblio.new(['Nitesh'],['Gul'],'Cloud9', 'LPP', 1404, 'December 9 2015', ['9781937785499', '1937785491'], 'The Facets of Ruby')
         @biblio3 = Biblio.new(['Scott'],['Chacon'], 'Pro Git 2009th Edition', 'Apress', 2009, 'August 27 2000', ['9781430218333','1430218339'], 'Pro')
         @biblio4 = Biblio.new(['Scott'],['Chacon'], 'Abecedario', 'Apress', 2009, 'August 27 2000', ['9781430218333','1430218339'], 'Pro')
         @revista1 = Revista.new(['Dave', 'Andy'], ['Thomas', 'Hunt'], 'programming ruby', 'Pragmatic Bookshelf', 4, 'July 7 2013', ['9781937785499', '1937785491'], 'The Facets of Ruby')
         @periodico1 = Periodico.new(['Maria'], ['Porr'],'Noticias', 'Diario de avisos', 1, ['9781937785499', '1937785491'], 'February 10 2013', 15)
         @apa1 = APA.new(@biblio1)
         @apa2 = APA.new(@revista1)
         @apa3 = APA.new(@biblio2)
         @apa4 = APA.new(@biblio3)
         @apa5 = APA.new(@biblio4)
         @apa6 = APA.new(@periodico1)
         @lista1 = Lista.new()
         @todos = Orden.new()
    end
    
    describe "pruebas" do
        it "Nombre y apellido (diversos autores)" do
           expect(@apa1.nomape).to eq("Autor, T. D. & Autor, H. A.")
        end
        it "Nombre y apellido (un solo autor)" do
           expect(@apa4.nomape).to eq("Autor, C. S.")
        end
        it "Ordenar" do
            @lista1.insertarfinish(@biblio2)
            @lista1.insertarfinish(@biblio1)
            @lista1.sort
            expect(@lista1.extraerstart).to eq(@biblio2)
            expect(@lista1.extraerstart).to eq(@biblio1)
        end
    end
    
    describe "comprobando las mayusculas" do
        it "mayuscula (la primera letra de cada palabra del título)" do
            expect(@apa2.get_titulo).to eq('Programming Ruby')
        end
    end
end
