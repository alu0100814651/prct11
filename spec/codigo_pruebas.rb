...
describe Biblio do
  ...
  describe Cola do
    before :all do
        ...
        @bib1 = Biblio.new(['Dave Thomas','Andy Hunt','Chad Fowler'], 'Programming Ruby 1.9 & 2.0: The Pragmatic Programmers’ Guide', 'Pragmatic Bookshelf', 4, 'July 7 2013', ['9781937785499', '1937785491'], 'The Facets of Ruby')
        @bib2 = Biblio.new('Scott Chacon', 'Pro Git 2009th Edition', 'Apress', 2009, 'August 27 2009', ['9781430218333','1430218339'], 'Pro')
        @bib3 = Biblio.new(['David Flanagan','Yukihiro Matsumoto'], 'The Ruby Programming Language', 'O’Reilly Media', 1, 'February 4 2008', ['0596516177','9780596516178'])
        @bib4 = Biblio.new(['David Chelimsky','Dave Astels','Bryan Helmkamp','Dan North','Zach Dennis','Aslak Hellesoy'], 'The RSpecBook: Behaviour Driven Development with RSpec, Cucumber, and Friends', 'Pragmatic Bookshelf', 1, 'December 25 2010', ['1934356379','9781934356371'], 'The Facets of Ruby')
    ...
    describe "Insertar mas de un elemento en la cola" do
        it "Podemos insertar mas de un elemento en la cola" do
            ...
            expect(@cola1.insertar(@bib1)).to eq(true)
            expect(@cola1.insertar(@bib2)).to eq(true)
            expect(@cola1.insertar(@bib3)).to eq(true)
            expect(@cola1.insertar(@bib4)).to eq(true)
            expect(@cola1.insertar(@bib5)).to eq(true)
        end
    end
    describe "Se deberia poder extraer mas de un elemento de la cola" do
        it "Podemos extraer mas de un elemento de la cola" do
            ...
            expect(@cola1.extraer).to eq(@bib1)
            expect(@cola1.extraer).to eq(@bib2)
            expect(@cola1.extraer).to eq(@bib3)
            expect(@cola1.extraer).to eq(@bib4)
            expect(@cola1.extraer).to eq(@bib5)
        end
    end
